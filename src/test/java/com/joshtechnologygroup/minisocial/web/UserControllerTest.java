...
class UserControllerTest {
    ...
    void createUser_shouldReturn422_whenMissingFields() throws Exception {
        UserCreateRequest userCreateRequest =
            UserFactory.defaultUserCreateRequest().email(null).build();
        mockMvc
            .perform(
                post("/api/user")
                    .contentType(MediaType.APPLICATION_JSON)
    ...
    @Test
    @WithMockUser(username = "john.doe@test.com")
    void partiallyUpdateUser_shouldReturn422_whenInvalidEmail()
        throws Exception {
        UserUpdateRequest userUpdateRequest = UserUpdateRequest.builder()
            .email("invalid-email")
            .build();
    ...
    void getUsers_shouldFilterByAgeRange_whenMinMaxAgeProvided()
        ...
        assert (list
                ...
                .allMatch(
                    user ->
                        user.userDetails().age() >= 28 &&
                        user.userDetails().age() <= 32
                ));
    }
    ...
    void getUsers_shouldFilterByResidentialCities_whenResidentialCitiesProvided()
        ...
        assert (list
                .stream()
                .anyMatch(user ->
                    user.userDetails().firstName().equals("John")
                ));
        assert (list
                .stream()
                .anyMatch(user ->
                    user.userDetails().firstName().equals("Mike")
                ));
    }
    ...
    void getUsers_shouldFilterByFirstNameAndLastName_whenNameFiltersProvided()
        ...
        assert (list
                .stream()
                .anyMatch(user ->
                    user.userDetails().firstName().startsWith("J")
                ));
    }
    ...
    void getUsers_shouldFilterByActiveStatus_whenActiveFilterProvided()
        ...
        assert (list
                .stream()
                .anyMatch(user ->
                    user.userDetails().firstName().equals("Jane")
                ));
        assert (list
                .stream()
                .anyMatch(user ->
                    user.userDetails().firstName().equals("Sarah")
                ));
    }
    ...
    void getUsers_shouldApplyComplexFiltering_whenMultipleFiltersProvided()
        ...
        assert (list
                ...
                .allMatch(
                    user ->
                        user.userDetails().gender().toString().equals("MALE") &&
                        user.userDetails().age() >= 25 &&
                        user.userDetails().age() <= 35
                ));
    }
