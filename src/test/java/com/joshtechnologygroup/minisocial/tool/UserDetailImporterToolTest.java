...
import com.joshtechnologygroup.minisocial.repository.*;
...
class UserDetailImporterToolTest {
    ...
    void userDetailImporter_shouldImportData_whenValid() {
        String userDetailsCsv = Objects.requireNonNull(
            getClass().getClassLoader().getResource("user-details.csv")
        ).getPath();
        String followingDetailsCsv = Objects.requireNonNull(
            getClass().getClassLoader().getResource("following-details.csv")
        ).getPath();

        try {
            userDetailImporterTool.importUserDetails(
                userDetailsCsv,
                followingDetailsCsv,
                "50"
            );
            assertEquals(10, userRepository.findAll().size());
            assertEquals(10, userDetailRepository.findAll().size());
            assertEquals(10, residentialDetailRepository.findAll().size());
            assertEquals(10, officialDetailRepository.findAll().size());
            Optional<User> arjun = userRepository.findByEmail(
                "arjun.nair@zenithcorp.in"
            );
            assertTrue(arjun.isPresent());
            assertEquals(4, userFollowRepository.findFollowedIdsByEmail(arjun.get().getEmail()).size());
        } catch (Exception e) {
            // Fail the test if any exception occurs
            fail(
                "Importing valid data should not throw an exception: " +
                    e.getMessage()
            );
        }
    }
    ...
    void userDetailImporter_shouldHandleFileNotFound_whenUserDetailsFileDoesNotExist() {
        ...
        assertEquals(0, userDetailRepository.findAll().size());
        assertEquals(0, residentialDetailRepository.findAll().size());
        assertEquals(0, officialDetailRepository.findAll().size());
    }
    ...
    void userDetailImporter_shouldHandleFileNotFound_whenFollowingDetailsFileDoesNotExist() {
        ...
        String nonExistentFollowingDetailsCsv =
            "non-existent-following-details.csv";

        // This should not throw an exception but should handle the error gracefully
        assertDoesNotThrow(() -> userDetailImporterTool.importUserDetails(
            userDetailsCsv,
            nonExistentFollowingDetailsCsv,
            "50"
        ));
    ...
    void userDetailImporter_shouldHandleEmptyFile_whenUserDetailsFileIsEmpty() {
        String emptyUserDetailsCsv = Objects.requireNonNull(
            getClass().getClassLoader().getResource("user-details-empty.csv")
        ).getPath();
        String followingDetailsCsv = Objects.requireNonNull(
            getClass().getClassLoader().getResource("following-details.csv")
        ).getPath();
    ...
    void userDetailImporter_shouldHandleInvalidData_whenCsvContainsInvalidValues() {
        String invalidUserDetailsCsv = Objects.requireNonNull(
            getClass().getClassLoader().getResource("user-details-invalid.csv")
        ).getPath();
        String followingDetailsCsv = Objects.requireNonNull(
            getClass().getClassLoader().getResource("following-details.csv")
        ).getPath();
    ...
    void userDetailImporter_shouldHandleMalformedCsv_whenCsvHasMissingColumns() {
        String malformedUserDetailsCsv = Objects.requireNonNull(
            getClass().getClassLoader().getResource("user-details-bad.csv")
        ).getPath();
        String followingDetailsCsv = Objects.requireNonNull(
            getClass().getClassLoader().getResource("following-details.csv")
        ).getPath();
